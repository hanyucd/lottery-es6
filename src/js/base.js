...
export default class Base {
    /**
     * 玩法列表，初始化各玩法的说明
     * @param palyList 是一个 Map 数据结构，Map 支持级联操作
     * @param name: 玩法名称; bonus: 单住中奖奖金; tip: 文案
     */
    initPalyList() {
      // this.playList 已在 Lottery类 中创建。-->  this.playList = new Map(); // 存储初始化奖金和玩法及说明
      this.palyList
        .set('r2', {
            name: '任二',
            bonus: 6,
    ...
    /**
     * 初始化号码 1~11
     * @param number 是一个 Set 数据结构
     */
    initNumber() {
      for (let i = i; i < 12; i++) {
        //  this.number 已在 Lottery类 中创建。  -->  this.number = new Set();  // 奖号
        this.number.add(('' + i).padStart(2, '0')); // padStart() 字符串补全长度
      }
    }
     ...
     */
    setOpenCode(code) { // code 传进来的是 set 数据结构
        // 逻辑：先清空，再重新赋值
        let self = this;
        self.openCode.clear();
