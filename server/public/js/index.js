	  ...
	  _createClass(Timer, [{
	    ...
	    value: function countDown(end, update, handle) {
	      ...
	      if (now - end) {
	        ...
	        var d = Math.floor(last_time / d_to_ms); // 计算 天
	        var h = Math.floor((last_time - d * d_to_ms) / h_to_ms); // 计算 小时
	        var m = Math.floor((last_time - d * d_to_ms - h * h_to_ms) / m_to_ms); // 计算 分钟
	        var s = Math.floor((last_time - d * d_to_ms - h * h_to_ms - m * m_to_ms) / s_to_ms); // 计算 秒
...
/***/ (function(module, exports) {
	...
	var Calculate = function () {
	  ...
	  _createClass(Calculate, [{
	     ...
	     */
	    value: function calcCount(activeCount, playType) {
	      var count = 0; // 默认注数为 0
	      var exist = this.play_list.has(playType); // play_list是 map数据结构，有 has 来判断 | 判断玩法列表中是否存在该玩法
	    ...
	    key: 'calcBonus',
	    value: function calcBonus(activeCount, playType) {
	      var self = this;
	      var play = playType.split('');
	      var arr = new Array(play[1] * 1).fill(0);
	      var min = void 0,
	          max = void 0;

	      if (play[0] === 'r') {
	        var min_active = 5 - (11 - activeCount); // 最小命中数
	        if (min_active > 0) {
